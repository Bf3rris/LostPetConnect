...
if($stmt->execute()){$result = $stmt->get_result();
					$array = $result->fetch_assoc();
					 ...
					 $auth_token = getenv('SIGNALWIRE_AUTH_TOKEN');
					 
					 //Var holding signalwire phone umber
...
if($reg_count == 0){
	...
	/////unregistered phone number sstuff
		...
		$stmt2 = $conn->prepare($retrieve);
		$stmt2->bind_param('s', $explode[1]);
  ...
  $client = new Client($project_id, $auth_token, array("signalwireSpaceUrl" => $space_url));
  
		//Sending acknowledgement to locator of pet
		$message = $client->messages
                    ->create($from, // to
                             array("from" => $xfn, "body" => "Thank you for your report! The owner has been notified. Please standby.")
                    );
		
		//Initial ping to pet owner notifying of pet being found
		 $message = $client->messages
                    ->create("+1".$phone_number, // to
                             array("from" => $xfn, "body" => "[Lost Pet Connect] $pet_name has been located.")
                    );
		
		//Message to pet owner with locators phone number
		 $message = $client->messages
                    ->create("+1".$phone_number, // to
                             array("from" => $xfn, "body" => "Locators #: $from")
                    );
