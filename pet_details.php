...
if($_SESSION['state'] == "Select your state:"){$_SESSION['stateerror'] = "Select a state"; header("location: registration.php"); exit;}	
	

//Checking if password fields contains at least 6 chars
if(strlen(strip_tags($_POST['password'])) < 6){$_SESSION['passwordlength'] = "Password is less than 6 characters"; header("location: registration.php"); exit;}
if(strlen(strip_tags($_POST['confirmpassword'])) < 6){$_SESSION['passwordlength'] = "Password is less than 6 characters"; header("location: registration.php"); exit;}
...
if($password != $confirm_password){$_SESSION['passwordmatch'] = "Password fields don't match"; header("location: registration.php"); exit;}

	//Checking for empty password fields
if(empty($password)){$_SESSION['passwordoneerror'] = "Password is empty"; header('location: registration.php'); exit;}
if(empty($confirm_password)){$_SESSION['passwordtwoerror'] = "Confirm Password is empty"; header('location: registration.php'); exit;}	

	
//Directing back to registration page if password fields don't match
if($password != $confirm_password){$_SESSION['passworderror'] = "<font color='#ff0000'>Passwords don't match</font>"; header('location: registration.php'); exit;}else{$_SESSION['password'] = $password;}
...
if($count > 0){
	...
	$_SESSION['emailexistserror'] = "<small>Email address already exists in database</small>"; header("location: registration.php");
 exit;
}}else{
	
	//Setting session var for invalid email address error
	$_SESSION['invalidemail'] = "Email address isn't valid"; header("location: registration.php"); exit;}


}

//Checking for empty fields / directing to registration page to complete empty fields
if(empty($_SESSION['firstname'])){$_SESSION['firstnameerror'] = "<font color='#ff0000'>First name is blank</font>"; header("location: registration.php"); exit;}
if(empty($_SESSION['lastname'])){$_SESSION['lastnameerror'] = "<font color='#ff0000'>Last name is blank</font></font>"; header("location: registration.php"); exit;}
if(empty($_SESSION['email_address'])){$_SESSION['emailerror'] = "<font color='#ff0000'>email address is blank</font>"; header("location: registration.php"); exit;}
if(empty($_SESSION['mobilenumber'])){$_SESSION['mobilenumbererror'] = "<font color='#ff0000'>Phone number is empty</font>"; header("location: registration.php"); exit;}
if(empty($_SESSION['city'])){$_SESSION['cityerror'] = "<font color='#ff0000'>City is blank</font></font>"; header("location: registration.php"); exit;}
if(empty($_SESSION['state'])){$_SESSION['stateerror'] = "<font color='#ff0000'>State is blank</font>"; header("location: registration.php"); exit;}
if(empty($_SESSION['zip'])){$_SESSION['ziperror'] = "<font color='#ff0000'>Zip code is empty</font>"; header("location: registration.php"); exit;}
...
<head>
<meta charset="utf-8">
<title>Lost Pet Connect - Registration / Pet Details</title>
<style type="text/css">
body,td,th {
	font-family: Arial;
}
  ...
  <tbody>
    ...
    <tr>
      ...
      <td width="78%">
        ...
        <input type="text" name="petname" value="<?php if($_SESSION['petname'] != ""){echo $_SESSION['petname'];} ?>"></td>
      </tr>
    <tr>
      <td colspan="2">&nbsp;</td>
      </tr>
    ...
    <tr>
      ...
      <td><?php
		  ...
		  if($_SESSION['petageerror'] != ""){
	  echo $_SESSION['petageerror'];
			  //Unsetting error message
unset($_UNSET['petageerror']);}
 ?>
    ...
    <tr>
      ...
      <td><input type="number" name="petage"  max="99" min="1" value="<?php
		  ...
		  if($_SESSION['petage'] != ""){echo $_SESSION['petage'];}?>"></td>
      </tr>
    <tr>
      <td colspan="2">&nbsp;</td>
      </tr>
    ...
    <tr>
      <td colspan="2" align="center">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tbody>
            <tr>
              <td width="82" align="center"><a href="registration.php"><input type="button" value="&larr; Back"></a></td>
              <td width="77">&nbsp;</td>
              <td width="22">&nbsp;</td>
              <td width="22">&nbsp;</td>
